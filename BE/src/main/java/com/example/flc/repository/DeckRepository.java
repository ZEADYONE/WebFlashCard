...
public interface DeckRepository extends JpaRepository<Deck, Long> {
    ...
    Page<Deck> findPublicActiveDeck(Pageable pageable);
    ...
    @Query("SELECT d FROM Deck d " +
            ...
            "AND d.scope = 'Public' " +
            "AND r.name = 'ADMIN'")
    Page<Deck> findDeckCourse(Pageable pageable);
    ...
    @Query(value = "SELECT d.id, d.title, d.image, d.des, d.scope, u.user_name, " +
            ...
            "WHERE ((d.user_id = :userId) OR (d.scope = 'Public' AND d.user_id != :userId AND p.id IS NOT NULL )) " +
            "AND d.status = true " +
            ...
            nativeQuery = true)
    Page<Object[]> findDeckProgressRaw(@Param("userId") Long userId, Pageable pageable);
